...
class SongListComponent extends Component {
    ...
    deleteItemsClicked() {
        ...
        if (x.length > 0) {
            ...
            if (x.length > 1)
            {
                msg = "Пожалуйста подтвердите удаление " + x.length + " песен";
            }
            else
            {
                msg = "Пожалуйста подтвердите удаление песни " + x[0].name;
            }
    ...
    refreshItems(cp) {
        BackendService.retrieveAllSongs(cp, this.state.limit)
            .then(
                resp => {
    ...
    onDelete()
    {
        BackendService.deleteSongs(this.state.selected_items)
