...
class ShopListComponent extends Component {
    ...
    deleteItemsClicked() {
        ...
        if (x.length > 0) {
            ...
            if (x.length > 1)
            {
                msg = "Пожалуйста подтвердите удаление " + x.length + " магазинов";
            }
            else
            {
                msg = "Пожалуйста подтвердите удаление магазина " + x[0].name;
            }
    ...
    refreshItems(cp) {
        BackendService.retrieveAllShops(cp, this.state.limit)
            .then(
                resp => {
    ...
    onDelete()
    {
        BackendService.deleteShops(this.state.selected_items)
